...
namespace Shadowdactyl\Http\Controllers\Api\Remote\Servers;
...
use Shadowdactyl\Exceptions\Http\HttpForbiddenException;
use Shadowdactyl\Repositories\Eloquent\ServerRepository;
use Shadowdactyl\Events\Server\Installed as ServerInstalled;
use Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
use Shadowdactyl\Http\Requests\Api\Remote\InstallationDataRequest;
...
class ServerInstallController extends Controller
{
    ...
    public function store(InstallationDataRequest $request, string $uuid): JsonResponse
    {
        ...
        if ($isInitialInstall && config()->get('shadowdactyl.email.send_install_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        } elseif (!$isInitialInstall && config()->get('shadowdactyl.email.send_reinstall_notification', true)) {
            $this->eventDispatcher->dispatch(new ServerInstalled($server));
        }
